...
import com.prachatech.appointment.adapters.StatusAdapter;
...
//            clearAll.setVisibility(View.VISIBLE);
    ...
    private void fetchAppList(final int types) {
     ...
     if(types==stausListApi){
         call= service.appList(object, ApiUrl.context_type);
     }else if(types==historyListApi){
         call= service.appHistoryList(object, ApiUrl.context_type);
     }
        showDialog();
    ...
    private void apiAccept(AppStatus status, String s) {
        if(isConnected()){
            ...
            Call<RegResult> call=service.accept(object,ApiUrl.context_type);
            showDialog();
